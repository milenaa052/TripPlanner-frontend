...
function TelaPasseios() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3000/viagem/${id}`, {
        headers: {
  ...
  const carregarPasseios = () => {
    axios
      .get(`http://localhost:3000/passeios/?viagemId=${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      })
  ...
  const deletarPasseio = (idPasseio: number) => {
    axios
      .delete(`http://localhost:3000/passeio/${idPasseio}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      })
    ...
    <div>
      <div className="agenda">
        {diasExibidos.map((dia) => (
          <button
            key={dia}
            className={`dia ${diaSelecionado === dia ? "ativo" : ""}`}
      ...
      <div className="paginacao">
        ...
        <button
          ...
          disabled={indexFinal >= diasViagem.length}
          className="botao"
        >
      ...
      <div className="botao">
        <div className="adicionar">
          <a
            href={`/cadastro-passeio/${id}?data=${converterData(
              diaSelecionado
            )}`}
            className="link"
          >
